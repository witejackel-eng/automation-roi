...
import { CheckCircle2, Clock, AlertCircle, Zap, Eye, BarChart3, FileText, TrendingUp, Shield } from "lucide-react"

const ALL_STEPS = [
  { id: 145, title: "Lead-gen n8n workflow",     agent: "modeling",       status: "complete", verdict: "BUILD",      confidence: 78, roi: "186%",  time: "Just now" },
  { id: 144, title: "CRM sync Make.com",          agent: "stress-test",    status: "complete", verdict: "BUILD",      confidence: 82, roi: "210%",  time: "1m ago" },
  { id: 143, title: "Zapier order automation",    agent: "scenario-comp",  status: "running",  verdict: "CONSIDER",   confidence: 52, roi: "94%",   time: "1m ago" },
  { id: 142, title: "AI agent onboarding flow",   agent: "confidence-sc",  status: "complete", verdict: "BUILD",      confidence: 71, roi: "152%",  time: "2m ago" },
  { id: 141, title: "Invoice processing custom",  agent: "break-even",    status: "approved", verdict: "DON\u2019T BUILD", confidence: 35, roi: "28%",   time: "8m ago" },
  { id: 140, title: "Email sequence Zapier",      agent: "modeling",       status: "running",  verdict: "BUILD",      confidence: 88, roi: "340%",  time: "22m ago" },
  { id: 139, title: "Appointment booking n8n",    agent: "stress-test",    status: "complete", verdict: "BUILD",      confidence: 74, roi: "168%",  time: "1h ago" },
]

const ALL_REVIEW_FILES = [
  { file: "inputs/hourly-cost.ts",     pct: 72 },
  { file: "inputs/leads-per-month.ts", pct: 45 },
  { file: "engine/scenario.ts",       pct: 88 },
  { file: "engine/stress-test.ts",    pct: 31 },
  { file: "engine/confidence.ts",     pct: 60 },
]

const ALL_REVIEW_LINES: { type: "comment"|"approve"|"change"|"code"; text: string; author?: string }[] = [
  { type: "code",    text: "const scenario = calculateScenario(inputs, 'expected')" },
  { type: "comment", text: "Conservative case passes all three gates", author: "modeling" },
  { type: "code",    text: "const verdict = recommend(scenario.expected)" },
  { type: "approve", text: "Verdict: BUILD — ROI 186%, payback 4.2mo", author: "stress-test" },
  { type: "code",    text: "const confidence = computeConfidenceScore(statuses)" },
  { type: "change",  text: "3 estimated inputs reduce score to 78", author: "confidence-sc" },
  { type: "code",    text: "const breakEven = computeBreakEven(inputs, 'expected')" },
  { type: "approve", text: "Answer holds until fee > $18,400", author: "break-even" },
  { type: "code",    text: "export const businessCase = generateReport(all)" },
  { type: "comment", text: "Client document ready for review", author: "modeling" },
  { type: "code",    text: "return { status: 'done', verdict: 'BUILD' }" },
  { type: "approve", text: "All checks pass — ship it", author: "stress-test" },
]

const COMMITS = [
  { hash: "a3f8c21", msg: "feat: add 64-permutation stress test engine",   time: "Just now" },
  { hash: "b7d2e09", msg: "feat: confidence scoring with 3-tier weights",    time: "4m ago"   },
  { hash: "c9a1f34", msg: "fix: break-even fee calculation edge case",       time: "12m ago"  },
  { hash: "d4e6b78", msg: "perf: reduce scenario computation by 40%",         time: "31m ago"  },
  { hash: "e2c9d56", msg: "feat: client-ready PDF with input tracing",        time: "1h ago"   },
]
...
function VerdictBadge({ verdict }: { verdict: string }) {
  const cfg = {
    BUILD:         { bg: "rgba(13,107,63,0.1)",   color: "#0D6B3F", icon: <CheckCircle2 style={{ width: 9, height: 9 }} />, label: "BUILD" },
    CONSIDER:      { bg: "rgba(139,94,10,0.12)",  color: "#8B5E0A", icon: <AlertCircle  style={{ width: 9, height: 9 }} />, label: "CONSIDER" },
    "DON’T BUILD": { bg: "rgba(155,10,46,0.1)",  color: "#9B0A2E", icon: <AlertCircle  style={{ width: 9, height: 9 }} />, label: "DON\u2019T BUILD" },
    approved:      { bg: "rgba(13,107,63,0.1)",   color: "#0D6B3F", icon: <CheckCircle2 style={{ width: 9, height: 9 }} />, label: "Approved" },
  }[verdict] ?? { bg: "#eee", color: "#666", icon: null, label: verdict }
...
function ReviewLine({ item, delay }: { item: typeof ALL_REVIEW_LINES[0]; delay: number }) {
  ...
  const iconCfg = {
    approve: { icon: <CheckCircle2 style={{ width: 9, height: 9, color: "#0D6B3F", flexShrink: 0 }} />, color: "#0D6B3F" },
    change:  { icon: <AlertCircle  style={{ width: 9, height: 9, color: "#8B5E0A", flexShrink: 0 }} />, color: "#8B5E0A" },
    comment: { icon: <Eye style={{ width: 9, height: 9, color: "rgba(0,0,0,0.35)", flexShrink: 0 }} />, color: "rgba(0,0,0,0.5)" },
...
export function AnalysisInterface({ revealDelay = 0 }: { revealDelay?: number }) {
  ...
  useEffect(() => {
    if (!mounted) return
    const t = setInterval(() => setStepOffset(v => (v + 1) % (ALL_STEPS.length - 3)), 4000)
    return () => clearInterval(t)
  }, [mounted])
  ...
  const visibleSteps = ALL_STEPS.slice(stepOffset, stepOffset + 4)
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          }}>viableo / roi-engine — main</span>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
            <LiveDot />
            <span style={{ fontSize: 8, color: "rgba(40,167,69,0.8)", letterSpacing: "0.16em", fontFamily: "monospace" }}>ENGINE OPERATIONAL</span>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(251,250,247,0.9)" }}>
          {[
            { label: "Cases run today",   val: 18,       icon: <FileText style={{ width: 11, height: 11 }} />,   graph: <MiniBarGraph seed={0} /> },
            { label: "Verdicts rendered", val: 34,       icon: <Shield style={{ width: 11, height: 11 }} />,        graph: <MiniBarGraph seed={5} /> },
            { label: "Stress tests",       val: 127,      icon: <TrendingUp style={{ width: 11, height: 11 }} />,    graph: <MiniDotGraph seed={2} /> },
            { label: "Analyses / min",     val: reqCount, icon: <Zap style={{ width: 11, height: 11 }} />,          graph: <LiveSparkline seed={7} /> },
          ].map((m, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 0.85fr", gap: 8, padding: 8, height: 340, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(160) }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <BarChart3 style={{ width: 10, height: 10, color: "rgba(0,0,0,0.38)" }} />
                <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Analysis Queue</span>
              </div>
              <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.25)", fontFamily: "monospace" }}>{ALL_STEPS.filter(s => s.status === "running").length} ACTIVE</span>
            </div>
            <div style={{ position: "relative", overflow: "hidden", flex: 1 }}>
              {visibleSteps.map((step, i) => (
                <div key={`${step.id}-${stepOffset}`} style={{
                  ...panel,
                  padding: "8px 10px",
                  marginBottom: 5,
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 5, marginBottom: 5 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 9.5, fontWeight: 600, color: "#111", lineHeight: 1.3, marginBottom: 2 }}>{step.title}</div>
                      <div style={{ fontSize: 7.5, fontFamily: "monospace", color: "rgba(0,0,0,0.32)" }}>{step.agent} · #{step.id}</div>
                    </div>
                    <VerdictBadge verdict={step.verdict} />
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", gap: 7 }}>
                      <span style={{ fontSize: 7.5, color: "#0D6B3F", fontFamily: "monospace" }}>ROI {step.roi}</span>
                      <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.35)", fontFamily: "monospace" }}>Conf {step.confidence}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                      <Clock style={{ width: 7, height: 7, color: "rgba(0,0,0,0.22)" }} />
                      <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.28)", fontFamily: "monospace" }}>{step.time}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{ ...panel, padding: "8px 10px", flexShrink: 0, height: 76, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
                <Zap style={{ width: 9, height: 9, color: "rgba(0,0,0,0.33)" }} />
                <span style={{ fontSize: 7.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.33)", fontFamily: "monospace" }}>Analysis Activity</span>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(210) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Engine Review — #{ALL_STEPS[reviewFileIdx % ALL_STEPS.length].id}</span>
            </div>
            <div style={{ ...panel, flex: 1, padding: "9px 10px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              <div style={{ marginBottom: 7, paddingBottom: 7, borderBottom: "1px solid rgba(0,0,0,0.05)", flexShrink: 0 }}>
                <div style={{ fontSize: 9.5, fontWeight: 600, color: "#111", marginBottom: 2 }}>Lead-gen n8n workflow</div>
                <div style={{ display: "flex", gap: 5 }}>
                  <span style={{ fontSize: 7.5, color: "#0D6B3F", fontFamily: "monospace" }}>ROI 186%</span>
                ...
                {ALL_REVIEW_FILES.map((f, i) => (
                  <div key={f.file} style={{ opacity: i === reviewFileIdx ? 1 : 0.55, transition: "opacity 0.4s ease" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      ...
                      <span style={{ fontSize: 7.5, fontFamily: "monospace", color: reviewFilePcts[i] > 70 ? "#0D6B3F" : "#8B5E0A", transition: "color 0.4s ease", fontWeight: i === reviewFileIdx ? 700 : 400 }}>{Math.round(reviewFilePcts[i])}%</span>
                    </div>
              ...
              <div style={{ borderTop: "1px solid rgba(0,0,0,0.05)", paddingTop: 7, flex: 1, overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 3, marginTop: 2 }}>
                  <span style={{ width: 4, height: 9, background: cursor ? "rgba(0,0,0,0.38)" : "transparent", transition: "background 0.08s", display: "inline-block" }} />
                </div>
              </div>
            </div>
          </div>

          {/* Col 3 — Audit Log + Stages */}
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(260) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              <FileText style={{ width: 10, height: 10, color: "rgba(0,0,0,0.38)" }} />
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Engine Commits</span>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "2px 2px 0", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Analysis Stages</span>
            </div>
            <div style={{ ...panel, flexShrink: 0, overflow: "hidden" }}>
              {[
                { name: "modeling",       status: "passing", duration: "1m 32s" },
                { name: "stress-test",     status: "running", duration: "0m 48s" },
                { name: "scenario-comp",   status: "passing", duration: "2m 11s" },
                { name: "confidence-sc",   status: "running", duration: "0m 54s" },
              ].map((a, i) => (
